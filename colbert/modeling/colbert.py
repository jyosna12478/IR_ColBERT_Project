...
def colbert_score_reduce_softmax(scores_padded, D_mask, config: ColBERTConfig):
    """
    Reduces scores using SoftMaxSim. Masks out padded values and applies softmax to the valid ones.
    """
    # Apply the mask: Set padded document scores to a large negative value (e.g., -inf)
    D_padding = ~D_mask.view(scores_padded.size(0), scores_padded.size(1)).bool()
    # Step 2: Apply masking and cast to float32 for stability
    
    # Expand to match scores shape: [B, D, Q]
    D_padding = D_padding.unsqueeze(-1).expand_as(scores_padded)  # [B, D, Q]

    # Softmax scoring with numerical stability
    scaled_scores = config.softmax_lambda * scores_padded
    scaled_scores = scaled_scores.masked_fill(D_padding, float('-inf'))
    
    # Stable softmax with log-sum-exp trick
    max_vals, _ = scaled_scores.max(dim=1, keepdim=True)  # [B, 1, Q]


    exps = torch.exp(scaled_scores - max_vals)
    exps = exps.masked_fill(torch.isnan(exps), 0.0)


    softmax_weights = exps / exps.sum(dim=1, keepdim=True)  # [B, D, Q]
    # Weighted sum over D tokens → [B, Q]
    softmaxsim = (softmax_weights * scores_padded).sum(dim=1)

    final_scores = softmaxsim.sum(dim=1)

    return final_scores
    
    # Apply softmax to the scores for each document, scaling by the softmax_lambda
    #softmax_weights = torch.nn.functional.softmax(config.softmax_lambda * scores_padded, dim=1)
   
    # Weighted sum over document tokens
     # Sum over the document tokens
...
def colbert_score(Q, D_padded, D_mask, config=ColBERTConfig()):
    ...
    print(f"[DEBUG] Using {'SoftMaxSim' if config.softmaxsim else 'MaxSim'} (λ = {config.softmax_lambda})")

    if config.softmaxsim:
    # Mask out padded values using -inf
        scores[~D_mask] = float('-inf')

    # Apply softmax over document tokens (dim=1)
        softmax_weights = torch.nn.functional.softmax(config.softmax_lambda * scores, dim=1)

    # Weighted sum over document tokens
        softmaxsim = (softmax_weights * scores).sum(dim=1)  # [batch_size, query_len]

    # Final score = sum over query tokens
        return softmaxsim.sum(dim=1)  # [batch_size]
    else:
...
def colbert_score_packed(Q, D_packed, D_lengths, config=ColBERTConfig()):
    ...
    """
    print(f"[DEBUG] colbert_score_packed() used — {'SoftMaxSim' if config.softmaxsim else 'MaxSim'} (λ = {config.softmax_lambda})")
    use_gpu = config.total_visible_gpus > 0
    ...
    if use_gpu or config.interaction == "flipr":
        ...
        print(f"[DEBUG] scores_padded shape: {scores_padded.shape}, scores_mask shape: {scores_mask.shape}")

        if config.softmaxsim:

            # Final score = sum over query tokens
            return colbert_score_reduce_softmax(scores_padded, scores_mask, config)
        else:
