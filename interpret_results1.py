...
softmax_ranking_path = "experiments/lotte/retrieval_softmaxsim_lambda_10_log/dev/writing.search.ranking.tsv"
...
for qid, retrieved in softmax_rankings.items():
    ...
    if has_relevant and shown_success < 3:
...
for qid, retrieved in softmax_rankings.items():
    ...
    if not has_relevant and shown_failure < 3:
        print(f"QID {qid}: Top 5 Results (SoftMaxSim): {top5}")
        print(f"Top 5 Results (MaxSim): {maxsim_rankings[qid][:5]}")
        print(f"Relevant: {relevant}")
        print("-" * 40)
